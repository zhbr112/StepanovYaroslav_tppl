...
#include "collector.hpp" // Подключаем нашу логику
...
public:
    ...
    bool connect_and_auth() {
        ...
        struct timeval tv;
        tv.tv_sec = SOCKET_TIMEOUT_SEC; tv.tv_usec = 0;
        ...
        if (send(sockfd_, AUTH_KEY.c_str(), AUTH_KEY.length(), 0) != (ssize_t)AUTH_KEY.length()) return false;
    ...
    void run_loop() {
        ...
        while (g_running) {
            ...
            while (g_running) {
                ...
                if (n <= 0) break; // Разрыв или ошибка
                ...
                if (accumulator.size() > 512) accumulator.clear(); // Защита от переполнения
            }
            close_socket();
        }
    }
